...
export class SceneManager {
  ...
  /** Triggers a brief white flash (docs/storm.md's "During Lightning" camera effect) - decayed frame-by-frame in update(). */
  flashLightning() {
    this._flashStrength = 1;
  }

  update(delta) {
    this._flashStrength = Math.max(0, this._flashStrength - delta * 12);

    this.scene.background.copy(this._baseBackground);
    if (this._flashStrength > 0) {
      this.scene.background.lerp(this._flashColor, this._flashStrength);
    }
  }
}
